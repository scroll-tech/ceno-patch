...
impl Fp {
    ...
    #[inline]
    const fn mul_internal(&self, rhs: &Fp) -> Fp {
    ...
    pub fn mul(&self, rhs: &Fp) -> Fp {
        ...
        let mut output = *self;
        // The guest target is little-endian. `repr(transparent)` makes this a
        // zero-copy view of zkcrypto's six Montgomery u64 limbs as 12 words.
        let output_words = unsafe { &mut *(&mut output as *mut Fp as *mut [u32; 12]) };
        let rhs_words = unsafe { &*(rhs as *const Fp as *const [u32; 12]) };
        ceno_syscall::syscall_bls12381_fp_mulmod(output_words, rhs_words);
        ceno_syscall::syscall_bls12381_fp_mulmod(output_words, &R_INV);
        output
    }
    ...
    #[inline]
    #[cfg(not(target_os = "zkvm"))]
    const fn square_internal(&self) -> Self {
